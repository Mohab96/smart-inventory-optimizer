...
async function productRevenueTrend(req, res, next) {
  try {
    ...
    if (topProducts.length === 0) {
      return res.json({
        message: "No product revenue data found for the specified year",
      });
    }
    ...
    monthlyData.forEach((data) => {
      ...
      if (productName && month) {
        result[productName][monthNames[month - 1]] = Number(
    ...
    console.error("Error fetching monthly product revenues:", error);
    next(
...
module.exports = productRevenueTrend;
