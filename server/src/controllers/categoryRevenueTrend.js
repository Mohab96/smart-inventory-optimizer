...
async function categoryRevenueTrend(req, res, next) {
  try {
    ...
    if (topCategories.length === 0) {
      return res.json({
        message: "No category revenue data found for the specified year",
      });
    }
    ...
    monthlyData.forEach((data) => {
      ...
      if (categoryName && month) {
        result[categoryName][monthNames[month - 1]] = Number(
          data._sum.revenueAmount.toFixed(2)
        );
      }
    });
    ...
    console.error("Error fetching monthly category revenues:", error);
    next(
...
module.exports = categoryRevenueTrend;
